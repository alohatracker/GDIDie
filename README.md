...
| `-Undo -Force` | Undo *without* a trustworthy `state.json`, using **documented Microsoft defaults** (`CDPSvc=Automatic`, `DoSvc=Manual`, `DiagTrack=Automatic`, `dmwappushservice=Manual`, `CDPUserSvc=Automatic`, all four policy values removed). These are defaults, **not** your original configuration. |
